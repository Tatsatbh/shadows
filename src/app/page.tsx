...
const trustLogos = ["Vercel", "ramp", "Notion", "retool", "Linear", "MongoDB.", "Brex", "Mercor"]
...
const testimonials = [
  {
    quote:
      "I used Shadows.sh to prep for a staff engineer loop and felt more confident in every round. The mock interviews were spot on.",
    name: "Alex Chen",
    role: "Senior Software Engineer",
    company: "Interviewing at Meta",
    tag: "Interview Candidate",
    initials: "AC",
  },
  {
    quote:
      "I use Shadows.sh to run realistic system design and behavioral mocks with my team. The reports help us give better feedback.",
    name: "Sarah Mitchell",
  ...
  {
    quote:
      "As a founder, I care about signal and speed. Shadows.sh helps our team evaluate better and move fast without compromising quality.",
    name: "Rohan Patel",
...
const stats: Array<{ value: string; label: string; Icon: LucideIcon }> = [
  { value: "12,842+", label: "Real-time interviews completed in the last 30 days", Icon: Users },
  { value: "8,961+", label: "Shareable reports generated by users", Icon: FileText },
  { value: "37%", label: "Average improvement in interview readiness score", Icon: Zap },
  { value: "120+", label: "Countries using Shadows.sh every week", Icon: Globe2 },
]
      ...
      <div className="mx-auto grid max-w-[1560px] gap-9 lg:grid-cols-[0.78fr_1.22fr] lg:items-start">
        <div className="max-w-[620px]">
          ...
          <h1 className="mt-6 text-[42px] leading-[1.08] text-white md:text-[50px] lg:text-[42px] xl:text-[50px] 2xl:text-[58px]">
            <Pixel>
              <span className="lg:whitespace-nowrap">NOT LEETCODE</span>
              <br />
              <span className="lg:whitespace-nowrap">IN SILENCE.</span>
              <br />
...
function TrustBand() {
  return (
    <div className="mx-auto mt-12 max-w-[1560px] border-t border-[#0b72ff]/24 pt-7">
      <div className="mb-6 flex items-center gap-6">
        <p className="shrink-0 text-xs font-semibold uppercase text-zinc-500">Trusted by engineers and teams at</p>
        <span className="h-px flex-1 bg-[#0b72ff]/24 shadow-[0_0_16px_rgba(0,112,255,0.2)]" />
      </div>
      <div className="grid grid-cols-2 gap-x-8 gap-y-5 text-zinc-500 sm:grid-cols-4 lg:grid-cols-9">
        {trustLogos.map((logo) => (
          <div key={logo} className="flex min-h-8 items-center text-xl font-semibold opacity-80 grayscale">
            {logo}
          </div>
        ))}
        <div className="flex min-h-9 items-center text-sm text-zinc-600">and thousands more</div>
      </div>
    </div>
  )
}
    ...
    <section id="social-proof" className="px-5 py-16 md:px-8 md:py-24">
      <div className="mx-auto max-w-[1480px]">
        <div className="grid gap-10 lg:grid-cols-[1.4fr_0.7fr] lg:items-end">
          <div>
            <SectionKicker>04. Social proof</SectionKicker>
            <h2 className="mt-5 max-w-5xl text-3xl leading-tight text-white md:text-5xl">
              <Pixel>
                TRUSTED BY PEOPLE WHO <span className="text-[#0b72ff]">ACTUALLY INTERVIEW._</span>
              </Pixel>
            </h2>
          </div>
          <p className="max-w-lg text-lg leading-8 text-zinc-300">
            From first-time candidates to hiring leaders, Shadows.sh helps real people show up sharper and hire with
            confidence.
          </p>
        </div>
        ...
        <div className="mt-9 grid gap-5 lg:grid-cols-3">
          {testimonials.map((testimonial) => (
            <article key={testimonial.name} className="rounded-[8px] border border-[#0b72ff]/30 bg-[#061635]/30 p-6 shadow-[0_0_38px_rgba(0,112,255,0.11)]">
              <div className="flex gap-1 text-[#0b72ff]" aria-label="Five star review">
                {Array.from({ length: 5 }).map((_, index) => (
                  <Star key={index} className="h-5 w-5 fill-current" />
                ))}
              </div>
              <p className="mt-6 min-h-[118px] text-base leading-8 text-white md:text-lg">&quot;{testimonial.quote}&quot;</p>
              <div className="mt-8 border-t border-[#0b72ff]/24 pt-6">
                <div className="flex items-center gap-4">
                  <span className="grid h-12 w-12 place-items-center rounded-full bg-gradient-to-br from-zinc-100 to-zinc-600 text-xs font-semibold text-black">
                ...
                <span className="mt-6 inline-flex rounded-[5px] border border-[#0b72ff]/45 px-3 py-1.5 text-sm text-[#58a0ff]">
                  {testimonial.tag}
                </span>
              </div>
            </article>
